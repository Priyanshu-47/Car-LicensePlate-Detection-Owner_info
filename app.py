...
def vehicle_information(number):
    request = rq.get(f"http://www.regcheck.org.uk/api/reg.asmx/CheckIndia?RegistrationNumber={number}&username=AnimeGalaxy7211")
...
def get_info():
    ...
    Number_Of_Seats=info['NumberOfSeats']
    location=info['Location']
    st=f'''Discription ====>{discription}+Engine_no====>
    {engine_no}+Registration Year ====>{res_year}+Registration date ====>{res_date}+
    Car Name ====>{car_name}+Car Model ====>{car_model}+
    Fuel Type ====>{fuel_type}+ No of Seats ====>{Number_Of_Seats}+
    Location ====>{location}'''
    return st
